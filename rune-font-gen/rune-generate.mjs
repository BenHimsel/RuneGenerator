...
export function createRuneSVG(seed) {
  ...
  // Parent SVG Element
  let runeSVGParent = createRuneSVGParent(seed);
  ...
  for (let i = 0; i < poiSet.length - 1; i++) {
    ...
    if (dotSkipIndex.dot && i == dotSkipIndex.dotIndex) {
      runeSVGParent = appendRuneSVGDot(poiSet, i, runeSVGParent);
      ...
      runeSVGParent = appendRuneSVGPath(poiSet, i, i + 1, runeSVGParent);
    }
    ...
    if (extraPathPOIs.has(i)) {
      ...
      randomPathSubset.forEach((j) => {
        if (!(j == dotSkipIndex.dot || j == dotSkipIndex.skip)) {
          runeSVGParent = appendRuneSVGPath(poiSet, i, j, runeSVGParent);
        }
      });
    }
  }
  ...
  if (innerRngConnected() > 0.5) {
    runeSVGParent = appendRuneSVGPath(
      poiSet,
      poiSet.length - 1,
      0,
      runeSVGParent
